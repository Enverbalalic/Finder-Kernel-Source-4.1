...
static void __subsystem_restart(struct subsys_data *subsys)
{
	...
	//add by yubin for system crash because modem restart
	if (restart_level == RESET_SUBSYS_COUPLED && !strcmp(subsys->name,"lpass"))
		data->use_restart_order = 0; 
...
static int __init subsys_restart_init(void)
{
	...
	restart_level = RESET_SUBSYS_COUPLED;  //changed by yubin, from RESET_SOC to RESET_SUBSYS_COUPLED
