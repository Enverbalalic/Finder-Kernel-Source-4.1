...
#ifdef CONFIG_VENDOR_EDIT //add by liuhd for bcm4330 wifi suspend
...
msmsdcc_runtime_suspend(struct device *dev)
{
	...
	if (mmc) {
		...
		{
			// changed by bosj. wifi current as 19ma.
#ifdef CONFIG_VENDOR_EDI //add by liuhd for bcm4330 wifi suspend		
