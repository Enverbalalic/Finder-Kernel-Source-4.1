...
void	AppRcpDemo( uint8_t event, uint8_t eventParameter)
{
    ...
    case	MHL_TX_EVENT_RCP_READY:
        ...
        if( (0 == (MHL_FEATURE_RCP_SUPPORT & eventParameter)) ){
            TX_DEBUG_PRINT(( "App: Peer does NOT support RCP\n" ));
        }
        if( (0 == (MHL_FEATURE_RAP_SUPPORT & eventParameter)) ){
            TX_DEBUG_PRINT(( "App: Peer does NOT support RAP\n" ));
        }
        if( (0 == (MHL_FEATURE_SP_SUPPORT & eventParameter)) ){
            TX_DEBUG_PRINT(( "App: Peer does NOT support WRITE_BURST\n" ));
        }
        ...
        if( SiiMhlTxRcpSend( rcpKeyCode )){
            TX_DEBUG_PRINT(("App: SiiMhlTxRcpSend (%02X)\n", (int) rcpKeyCode));
        }else{
            TX_DEBUG_PRINT(("App: SiiMhlTxRcpSend (%02X) Returned Failure.\n", (int) rcpKeyCode));
        }
    ...
    case	MHL_TX_EVENT_RCP_RECEIVED:
        ...
        #endif
        TX_DEBUG_PRINT(("App1: Received an RCP key code = %02X\n", (int)rcpKeyCode ));
...
{	
    ...
    if( 0 == mhl_state.initInt){
        mhl_state.initInt = 1;
        enable_irq(sii8334_PAGE_TPI->irq);
        TX_DEBUG_PRINT((KERN_INFO "%s:the value of mhl_state.initInt is %d", __func__, mhl_state.initInt));
        return;
    }
    #else
    ...
    if( 0 == mhlTxConfig.initInt){
        ...
        enable_irq(sii8334_PAGE_TPI->irq);
        TX_DEBUG_PRINT((KERN_INFO "%s:the value of mhlTxConfig.initInt is %d", __func__, mhlTxConfig.initInt));
        return;
    }
...
//    unsigned long lock_flags = 0;	 
    ...
    #endif
    //TX_DEBUG_PRINT(("The sii8334 interrupt handeler is working..\n");  
    TX_DEBUG_PRINT(("The most of sii8334 interrupt work will be done by following tasklet..\n"));  
    
    schedule_work(sii8334work);
    
    //TX_DEBUG_PRINT(("The sii8334 interrupt's top_half has been done and bottom_half will be processed..\n"));  
    #ifdef zy_dbg_irq
    //deleted by zhangyue on 2011-12-07 for debugging irq problems
    #else
...
static int EventThread(void *data)
{
    ...
    TX_DEBUG_PRINT(("%s EventThread starting up\n", MHL_DRIVER_NAME));
    
    while (true){
        if (kthread_should_stop()){
            TX_DEBUG_PRINT(("%s EventThread exiting\n", MHL_DRIVER_NAME));
            break;
        }
...
static int mhl_Sii8334_resume(struct i2c_client *cl)
{
    ...
    if(match_id(&mhl_Sii8334_idtable[5], cl))	{
	  ...
	  //added by zhangyue on 2011-12-15 for debugging the hpd problem end
        schedule_delayed_work(ResumeWork, HZ);    
    }
#endif
    return 0;
};
