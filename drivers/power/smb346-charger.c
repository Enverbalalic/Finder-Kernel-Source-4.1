...
#define CHARGING_TEOC_S 28800				//8 hours
...
#define CHARGE_TERMINATION_CURR 50
#define CHARGE_TERMINATION_CURR_HI_TEMP 100
...
static int smb346_set_charge_cool(struct smb346_charger_mux *msm_chg)
{
	...
	/* set Float voltage. */
	rc= smb346_update(msm_chg, FLOAT_VOTAGE_REGISTER, FLOAT_VOTAGE_4220MV, FLOAT_VOTAGE_MASK);	
	if (rc)
		return rc;	
...
static int charge_complete_detect(struct smb346_charger_mux *msm_chg) 
{
	...
	charge_end_counter++;
	if(smb346_battery_temp_region_get(msm_chg) == CV_BATTERY_TEMP_REGION__WARM)
		termination_current = CHARGE_TERMINATION_CURR_HI_TEMP;
	else
		termination_current = CHARGE_TERMINATION_CURR;
	...
	if (msm_chg->mEndCVTopoffCounter >= CHARGE_END_COUNT){
		    if(smb346_battery_temp_region_get(msm_chg) == CV_BATTERY_TEMP_REGION__NORMAL)
			{
				...
				if(smb346_battery_adc_params.current_batt_adc_voltage >= 4100) {
					ret = smb346_handle_charging_done(msm_chg);
					if (ret)
						return ret;
				} else {
				...
				if(smb346_battery_adc_params.current_batt_adc_voltage >= 4000) {
					ret = smb346_handle_charging_done(msm_chg);
					if (ret)
						return ret;
				} else {
...
extern void charger_type_detect(void);
static void handle_event__mpp11_irq_triggered(struct smb346_charger_mux *msm_chg)
{
	...
	pr_err("%s:smb346 mpp11 plugin interrupt>>>\n",__func__);
	charger_type_detect();
...
void smb346_chg_connected(enum chg_type chgtype)
{
	char *chg_types[] = {"STD DOWNSTREAM PORT(USB Charger)",
			...
			"INVALID",
			"ACCESSORY Charger"};
/*OPPO Jiangsm modify begin for print important log*/
#ifdef CONFIG_VENDOR_EDIT
	ERR_CHG("%s  Charger Type: #### :%s\n", __func__, chg_types[chgtype]);
#endif
...
//for test 
static  long debug_battery_temperature=300;
static unsigned long debug_charger_voltage=5000;
static unsigned long debug_battery_voltage=3700;
static unsigned long debug_battery_capacity= 30;
static unsigned long debug_charging_current=300;
static bool debug_enabled= false;
bool smb346_debug_enable_get(void)
{
	return debug_enabled;
}
void smb346_debug_enable_set(bool enable)
{
	debug_enabled = enable;
}
unsigned long smb346_debug_battery_capacity_get(void)
{
	return debug_battery_capacity;
}
void smb346_debug_battery_capacity_set(unsigned long batt_cap)
{
	debug_battery_capacity = batt_cap;
}
unsigned long smb346_debug_charging_current_get(void)
{
	return debug_charging_current;
}
void smb346_debug_charging_current_set(unsigned long chg_curr)
{
	debug_charging_current = chg_curr;
}
long smb346_debug_battery_temperature_get(void)
{
	return debug_battery_temperature;
}
void smb346_debug_battery_temperature_set(long dbg_batt_temp)
{
	debug_battery_temperature = dbg_batt_temp;
}
unsigned  long smb346_debug_charger_voltage_get(void)
{
	return debug_charger_voltage;
}
void smb346_debug_charger_voltage_set(unsigned long dbg_chg_vol)
{
	debug_charger_voltage = dbg_chg_vol;
}
unsigned  long smb346_debug_battery_voltage_get(void)
{
	return debug_battery_voltage;
}
void smb346_debug_battery_voltage_set(unsigned long dbg_batt_vol)
{
	debug_battery_voltage = dbg_batt_vol;
}
static ssize_t smb346_debug_enable_show(struct device *dev, struct device_attribute *attr, char *buf){
	return sprintf(buf, "%d\n", smb346_debug_enable_get() == true? 1: 0);
}
static ssize_t smb346_debug_enable_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size){
	unsigned long val = simple_strtoul(buf, NULL, 10);
	if (val == 0 )
		smb346_debug_enable_set(false);
	else
		smb346_debug_enable_set(true);
	return size;
}
static DEVICE_ATTR(smb346_debug_enable, 0666, smb346_debug_enable_show, smb346_debug_enable_store);
static ssize_t smb346_debug_battery_capacity_show(struct device *dev, struct device_attribute *attr, char *buf){
	return sprintf(buf, "%d\n", (int)smb346_debug_battery_capacity_get());
}
static ssize_t smb346_debug_battery_capacity_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size){
	unsigned long val = simple_strtoul(buf, NULL, 10);
	smb346_debug_battery_capacity_set(val);
	return size;
}
DEVICE_ATTR(smb346_debug_battery_capacity, 0666, smb346_debug_battery_capacity_show, smb346_debug_battery_capacity_store);
static ssize_t smb346_debug_charging_current_show(struct device *dev, struct device_attribute *attr, char *buf){
	return sprintf(buf, "%d\n", (int)smb346_debug_charging_current_get());
}
static ssize_t smb346_debug_charging_current_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size){
	unsigned long val = simple_strtoul(buf, NULL, 10);
	smb346_debug_charging_current_set(val);
	return size;
}
DEVICE_ATTR(smb346_debug_charging_current, 0666, smb346_debug_charging_current_show, smb346_debug_charging_current_store);
static ssize_t smb346_debug_battery_temperature_show(struct device *dev, struct device_attribute *attr, char *buf){
	return sprintf(buf, "%ld\n", smb346_debug_battery_temperature_get());
}
static ssize_t smb346_debug_battery_temperature_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size){
	long val = simple_strtol(buf, NULL, 10);
	smb346_debug_battery_temperature_set(val);
	return size;
}
static DEVICE_ATTR(smb346_debug_battery_temperature, 0666, smb346_debug_battery_temperature_show, smb346_debug_battery_temperature_store);
	
static ssize_t smb346_debug_charger_voltage_show(struct device *dev, struct device_attribute *attr,  char *buf){
	return sprintf(buf, "%lu\n", smb346_debug_charger_voltage_get());
}
static ssize_t smb346_debug_charger_voltage_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size){
	unsigned long val = simple_strtoul(buf, NULL, 10);
	smb346_debug_charger_voltage_set(val);
	return size;
}
static DEVICE_ATTR(smb346_debug_charger_voltage, 0666, smb346_debug_charger_voltage_show, smb346_debug_charger_voltage_store);

static ssize_t smb346_debug_battery_voltage_show(struct device *dev, struct device_attribute *attr,  char *buf){
	return sprintf(buf, "%lu\n", smb346_debug_battery_voltage_get());
}
static ssize_t smb346_debug_battery_voltage_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size){
	unsigned long val = simple_strtoul(buf, NULL, 10);
	smb346_debug_battery_voltage_set(val);
	return size;
}
static DEVICE_ATTR(smb346_debug_battery_voltage, 0666, smb346_debug_battery_voltage_show, smb346_debug_battery_voltage_store);
//end for test
#ifdef FTM_CHARGE_MODE_FEATURE
...
static int __devinit smb346_charger_probe(struct platform_device *pdev)
{
	...
	//enable_irq(smb346_chg.smb346_irq);
       ...
       if(smb346_chg.dev){
		...
		/*create charger voltage , battery voltage ,battery temperature for debug test*/
		ret = device_create_file(smb346_chg.dev, &dev_attr_smb346_debug_battery_temperature);
		if (ret < 0) {
			ERR_CHG("%s: creat smb346 debug_battery_temperature file failed ret = %d\n", __func__, ret);
			device_remove_file(smb346_chg.dev, &dev_attr_smb346_debug_battery_temperature);
		}
		ret = device_create_file(smb346_chg.dev, &dev_attr_smb346_debug_charger_voltage);
		if (ret < 0) {
			ERR_CHG("%s: creat smb346 debug charger voltage file failed ret = %d\n", __func__, ret);
			device_remove_file(smb346_chg.dev, &dev_attr_smb346_debug_charger_voltage);
		}
		ret = device_create_file(smb346_chg.dev, &dev_attr_smb346_debug_battery_voltage);
		if (ret < 0) {
			ERR_CHG("%s: creat smb346 debug battery voltage file failed ret = %d\n", __func__, ret);
			device_remove_file(smb346_chg.dev, &dev_attr_smb346_debug_battery_voltage);
		}
		ret = device_create_file(smb346_chg.dev, &dev_attr_smb346_debug_charging_current);
		if (ret < 0) {
			ERR_CHG("%s: creat smb346 debug  charging current file failed ret = %d\n", __func__, ret);
			device_remove_file(smb346_chg.dev, &dev_attr_smb346_debug_charging_current);
		}
		ret = device_create_file(smb346_chg.dev, &dev_attr_smb346_debug_battery_capacity);
		if (ret < 0){
			ERR_CHG("%s:create smb346 debug battery capacity file failed ret=%d\n", __func__, ret);
			device_remove_file(smb346_chg.dev, &dev_attr_smb346_debug_battery_capacity);
		}
		ret = device_create_file(smb346_chg.dev, &dev_attr_smb346_debug_enable);
		if (ret < 0) {
			ERR_CHG("%s: creat smb346 debug enable file failed ret = %d\n", __func__, ret);
			device_remove_file(smb346_chg.dev, &dev_attr_smb346_debug_enable);
		}
