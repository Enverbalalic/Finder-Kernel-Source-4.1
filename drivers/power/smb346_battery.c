...
extern long smb346_debug_battery_temperature_get(void);
extern unsigned long smb346_debug_battery_voltage_get(void);
extern unsigned long smb346_debug_charger_voltage_get(void);
extern unsigned long smb346_debug_battery_capacity_get(void);
extern unsigned long smb346_debug_charging_current_get(void);
extern bool smb346_debug_enable_get(void);
static void msm_batt_qury_adc_battery_params(struct smb346_battery_adc_params_type* smb346_battery__params)
{
    ...
    /*update battery info*/
if ( smb346_debug_enable_get() == false){
	...
	ERR_BATT("==========now we are in debug mode\n");
	smb346_battery__params->current_batt_adc_voltage = (int)smb346_debug_battery_voltage_get();
	smb346_battery__params->batt_temperature = (int)smb346_debug_battery_temperature_get();
	smb346_battery__params->vchg_mv = (int)smb346_debug_charger_voltage_get();
	smb346_battery__params->ichg_mv = (int)smb346_debug_charging_current_get();
	smb346_battery__params->batt_capacity = (int)smb346_debug_battery_capacity_get();
}
	return;
}
...
///
	...
	if (fulegague_capacity >= 30){
		...
		DEBUG_BATT("fulegague_voltage_avg = %d mv\n", fulegague_voltage_avg);
		...
		DEBUG_BATT("the voltage_capactiy the map from avg voltage is %d\n", voltage_capacity);
...
int updata_batt_params_fulegague_resume(void)
{
	...
	if (fulegague_capacity >= 30){
		...
		discharging_current = MAX_UINT - cur_battery_adc_params.ichg_mv;
		if (discharging_current <= DISCHARGING_CURRENT_200_MA)
			map_table = capacity_to_voltage_map_with_150ma;
		else if (discharging_current <= DISCHARGING_CURRENT_350_MA)
			map_table = capacity_to_voltage_map_with_300ma;
		...
		//get hte capacity that the avg voltage map form different discharging current
		for (index_cap = 0; index_cap < MAX_CAPACITY; index_cap++){
			if (map_table[index_cap] >= cur_battery_adc_params.current_batt_adc_voltage &&  
				cur_battery_adc_params.current_batt_adc_voltage < map_table[index_cap + 1 ] ){
				voltage_capacity = index_cap;
				break;
			}
		}
		...
		if (voltage_capacity - fulegague_capacity >= 0) {
			if(fulegague_capacity_last > fulegague_capacity) {
				if(voltage_capacity > fulegague_capacity_last)
					fulegague_capacity = fulegague_capacity_last;
				else
			...
			if(fulegague_capacity_last > voltage_capacity) {
				if(fulegague_capacity > fulegague_capacity_last)
					fulegague_capacity = fulegague_capacity_last;
				else
					fulegague_capacity = voltage_capacity;
