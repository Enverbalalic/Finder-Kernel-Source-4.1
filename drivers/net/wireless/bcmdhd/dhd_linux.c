...
//	int bcn_li_dtim = 3;
	...
	DHD_ERROR(("%s: bosj---enter, value = %d in_suspend=%d\n",
...
static int dhd_set_suspend_test(int value, dhd_pub_t *dhd)
{
	...
	#endif
				/* Kernel suspended */
				DHD_ERROR(("%s:bosj-- force extra Suspend setting \n", __FUNCTION__));
...
static void dhd_early_suspend(struct early_suspend *h)
{
	...
	//if (dhd)
		dhd_suspend_resume_helper(dhd_test, 1, 0);
}
...
dhd_concurrent_fw(dhd_pub_t *dhd)
{
    ...
    /* changed by bosj. the hardware test can`t be going. so this change. */
	printk("888888888888888--------------bosj------0000\n");
	return ret;
	//DHD_ERROR("------------------------------bosj-------99999\n");
	if ((!op_mode) && (strstr(fw_path, "_p2p") == NULL) &&
		...
		DHD_ERROR(("%s: ---bosj---9999999999999-----(error=%d)\n", __FUNCTION__, ret));
	}
	return ret;
}
...
*/
	...
	//printk("20130420--------------bosj------%ld\n", msecs_to_jiffies(dhd->wakelock_rx_timeout_enable));
			/* 
			 *	Changged by boshoujing. timeout(200) is setted by experience.
			 *  Maybe it has a problem. 
			*/
			wake_lock_timeout(&dhd->wl_rxwake,200);
