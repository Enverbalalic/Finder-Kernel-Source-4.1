...
/* OPPO 2012-09-25 wangjc Modify begin for reason */
#ifndef CONFIG_VENDOR_EDIT
...
#endif
/* OPPO 2012-09-25 wangjc Modify end */
...
/* OPPO 2012-09-19 wangjc Modify begin for reason */
#ifndef CONFIG_VENDOR_EDIT
...
{
	pr_err("%s:cancel charger detect\n", __func__);
	return;//do nothing
...
static bool startup_type_exec = false;
static void usb_chg_detect(struct work_struct *w);
void charger_type_detect(void)
{
	struct usb_info *ui = the_usb_info;
	
	if (startup_type_exec )
	return;
	pr_err("%s:type detect\n", __func__);
	if (ui == NULL)
		return;

	ui->attempts_remaining = DETECT_NUM_OF_ATTEMPTS;
	schedule_delayed_work(&ui->chg_det, USB_CHG_DET_POLL_DELAY);
}
...
#ifdef CONFIG_VENDOR_EDIT
    pr_err("%s:usb_chg_detect ready now!!!!\n", __func__);
#endif
/*OPPO Jiangsm modify end*/
	if(ui->attempts_remaining == 0)
	{
		pr_err("%s invalid attempts\n", __func__);
		return;
	}
...
#endif
/* OPPO 2012-09-19 wangjc Modify end */
