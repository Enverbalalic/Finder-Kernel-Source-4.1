...
struct pm8058_othc {
    //liuyan@MutimediaDrv.Headset begin
       ...
       bool hp_irq;              //prevent into interrupt twice
#endif  
...
#ifdef CONFIG_VENDOR_EDIT
extern int adc_get_vchg_mvolts(void); //liuyan@MutimediaDrv.Headset

//liuyan 2012-12-04 add,get the headset status
static ssize_t show_headset(struct device *dev, struct device_attribute *attr, char *buf);
static ssize_t store_headset(struct device *dev,struct device_attribute *attr, const char *buf, size_t count);
 static DEVICE_ATTR(headset_irq, S_IRUGO | S_IWUSR, show_headset, store_headset);
    static struct attribute *dev_attrs[] = {
            &dev_attr_headset_irq.attr,
            NULL,
    };
...
//yanghai remove
#ifndef CONFIG_VENDOR_EDIT// yanghai removed
...
#ifdef CONFIG_VENDOR_EDIT
	//liuyan 2012-12-04 add  get the headset irq status
	sysfs_remove_group(&dd->dev->kobj,&dev_attr_grp);
#endif
	kfree(dd);
...
//liuyan@MutimediaDrv.Headset begin
 #ifndef CONFIG_VENDOR_EDIT// liuyan removed
...
#ifdef CONFIG_VENDOR_EDIT  
	//liuyan modify, prevent into interruput twice at the same time
	spin_lock_irqsave(&dd->lock, flags);
       if(dd->hp_irq){
	dd->hp_irq=false;
       spin_unlock_irqrestore(&dd->lock, flags);
...
static irqreturn_t pm8058_nc_ir(int irq, void *dev_id)
{
	...
	if (rc < 0) {
      ...
      dd->othc_ir_state = !rc;
#ifndef CONFIG_VENDOR_EDIT  //liuyan modify in 2012-4-12
    dd->othc_ir_state = rc;
	if (dd->othc_ir_state) {
		/* disable irq, this gets enabled in the workqueue */
		disable_irq_nosync(dd->othc_irq_ir);
...
//liuyan 2012-12-4 add ,get headset irq status
                                                 ...
                                                 struct device_attribute *attr, char *buf)
{
	...
	ret = pm8xxx_read_irq_stat(dd->dev->parent, dd->othc_irq_ir);
	if (ret < 0) {
		pr_err("%s Unable to read IR status\n", __func__);
		return ret;
	}
...
static ssize_t store_headset(struct device *dev,
	                   struct device_attribute *attr, const char *buf, size_t count)
{
       ssize_t ret= strnlen(buf, PAGE_SIZE);
	uint8_t hp_irq = atoi(buf);
       struct pm8058_othc *dd = config[1];
	if(!dd)
		return -ENODEV;
	if(hp_irq){
        enable_irq(dd->othc_irq_ir);
	 pr_info("enable othc irq\n");
	}else{
        disable_irq_nosync(dd->othc_irq_ir);
	pr_info("disable othc irq\n");
	}
...
static int __devinit pm8058_othc_probe(struct platform_device *pd)
{
	...
	pr_debug("Device %s:%d successfully registered\n",
			pd->name, pd->id);
#ifdef CONFIG_VENDOR_EDIT
	//liuyan 2012-12-04 add  get the headset irq status /sys/bus/platform/devices/pm8058-othc.1/
	rc = sysfs_create_group(&dd->dev->kobj,
		&dev_attr_grp);
	if (rc) {
