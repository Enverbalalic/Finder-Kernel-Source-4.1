...
static void prox_work_func( struct work_struct *work )
{
	...
	{		
		...
		if (current_object == 0 && asl_data < (( saturation /5)*4))
		{
			...
			if(PD_temp == 1023)
			{
				PRINTK_IF(PRINT_LOG,"huyu----set PROX_UPPER_THRESHOLD = 800");
...
static int start_sensor( struct tsl2773_data *data)
{
    ...
    i2c_smbus_write_byte_data( data->client, TSL_CMD|TSL_REG_CONFIG/*0x8d*/, P_CYCLE_W );
    i2c_smbus_write_byte_data( data->client, TSL_CMD|TSL_REG_PPCOUNT/*0x8e*/, P_COUNT );
