...
static spinlock_t rw_lock_core;// 1219
#endif   
...
int hci_recv_frame(struct sk_buff *skb)
{
	...
	/* bt connect headset cause Kernel Panic...changed by bosj */
	#ifdef CONFIG_VENDOR_EDIT
	spin_lock_irqsave(&rw_lock_core, irq_flags);//20121219
	
	tasklet_schedule(&hdev->rx_task);
	spin_unlock_irqrestore(&rw_lock_core, irq_flags);
	#endif
